...
- **Persisted, searchable logs and traces** — runtime container logs for every deploy target and runtime family, plus OpenClaw structured gateway logs and native OTLP trace ingest, are captured into durable, encrypted, retention-bounded storage (`local` by default, or S3/R2), with search, export, per-workspace retention, a changeable storage destination, and a three-lens Runtime/Traces/Operator view on `/app/logs` — see the [logging guide](https://noradocs.solomontsao.com/guides/logging).
...
- **OpenTelemetry GenAI — available.** [OTLP + Prometheus export](https://noradocs.solomontsao.com/guides/opentelemetry) of runtime telemetry under the `gen_ai.*` semantic conventions — per-exchange chat spans plus token/cost/resource metrics flow into the Grafana / Datadog / Langfuse stack you already run. (Per-tool-call sub-spans are a separate, OpenClaw-only capability — agents push real traces into Nora's own [OTLP receiver](https://noradocs.solomontsao.com/guides/logging), viewable in the Traces lens on `/app/logs`.)
