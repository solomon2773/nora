#!/usr/bin/env bash
# infra-tests/run-all.sh — runs every implemented test script in order,
# writes one combined results/run-<timestamp>.jsonl, and prints a summary.
#
# Usage:
#   INFRA_TESTS_CONFIRM=yes-i-know ./run-all.sh                # everything
#   INFRA_TESTS_CONFIRM=yes-i-know ./run-all.sh phase4          # one phase dir
#   INFRA_TESTS_CONFIRM=yes-i-know ./run-all.sh phase4/01-worker-kill-midwindow.sh
#
# Each script also runs standalone (see any phaseN-*/README.md) — this is
# a convenience wrapper, not the only way to invoke a test. Running a
# single script directly while iterating on it is often faster than
