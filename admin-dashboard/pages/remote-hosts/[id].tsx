...
function accessAvailabilityLabel(host: RemoteHost, detailed = false) {
  if (host.availableToAll === true) {
    return detailed ? "Available to all accounts" : "all accounts";
  }
  if (host.availableToAll === false) {
    return detailed ? "Restricted grants" : "restricted";
  }
  return detailed ? "Access state unavailable" : "access unavailable";
}
...
export default function RemoteHostDetailPage() {
  ...
  const loadHost = useCallback(
    async (options: { background?: boolean } = {}) => {
      ...
      try {
        ...
        if (!response.ok) {
          const failure = await responseError(response, "Failed to load platform host");
          throw new Error(failure.message);
        }
        const nextHost = extractHost(await response.json().catch(() => ({})));
        if (!nextHost) throw new Error("Remote Host response was empty");
        setHost(nextHost);
        setNotFound(false);
        if (!options.background) setForm(buildRemoteHostForm(nextHost));
      } catch (error) {
        setActionError(errorMessage(error, "Failed to load platform host"));
      } finally {
  ...
  const loadAccess = useCallback(
    async (options: { notice?: string } = {}) => {
      ...
      try {
        ...
        if (failure) {
          const parsed = await responseError(failure, "Failed to load platform access");
        ...
        setAccessError(errorMessage(error, "Failed to load platform access"));
        setAccessLoaded(true);
        return false;
      } finally {
    ...
    [commitAccess, host, hostId, platformMode],
  );
  ...
  async function saveHost(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        const failure = await responseError(response, "Failed to save platform host");
        throw new Error(failure.message);
      }
      ...
      toast.success("Platform host configuration saved");
    } catch (error) {
      const message = errorMessage(error, "Failed to save platform host");
  ...
  async function testConnection() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error || "Remote Host connection test failed");
      }
      if (payload?.lastTestStatus === "failed") {
        throw new Error(payload?.lastTestMessage || "Remote Host connection test failed");
      }
      toast.success("SSH and Docker test passed; the host key is pinned");
      await loadHost();
    } catch (error) {
      const message = errorMessage(error, "Remote Host connection test failed");
      setActionError(message);
      toast.error(message);
  ...
  async function toggleEnabled() {
    ...
    try {
      ...
      if (!response.ok) {
        const failure = await responseError(response, "Failed to update platform host");
        throw new Error(failure.message);
      }
      toast.success(host.enabled === false ? "Platform host enabled" : "Platform host disabled");
      await loadHost();
    } catch (error) {
      const message = errorMessage(error, "Failed to update platform host");
      setActionError(message);
      toast.error(message);
    } finally {
      setToggling(false);
  ...
  async function resetSshPin() {
    ...
    try {
      ...
      if (!response.ok) {
        const failure = await responseError(response, "Failed to reset SSH pin");
        throw new Error(failure.message);
      }
      setConfirmAction(null);
      setConfirmation("");
      toast.success("SSH pin cleared. Test connection to trust the replacement host key.");
      await loadHost();
    } catch (error) {
      const message = errorMessage(error, "Failed to reset SSH pin");
      setActionError(message);
      toast.error(message);
    } finally {
      setResetting(false);
  ...
  async function deleteHost() {
    ...
    try {
      ...
      if (!response.ok) {
        const failure = await responseError(response, "Failed to delete platform host");
        const prefix = response.status === 409 ? "Delete conflict: " : "";
        throw new Error(`${prefix}${failure.message}`);
      }
      toast.success("Platform host deleted");
      await router.push("/remote-hosts");
    } catch (error) {
      const message = errorMessage(error, "Failed to delete platform host");
      setActionError(message);
      toast.error(message);
    } finally {
      setDeleting(false);
  ...
  async function saveAccessUpdate(next: RemoteHostAccess, busyKey: string) {
    ...
    if (!Number.isSafeInteger(before.version) || before.version < 1) {
      setAccessError("Access version is unavailable. Reload platform access before saving.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const failure = await responseError(response, "Failed to update platform access");
      ...
      const message = errorMessage(error, "Failed to update platform access");
    ...
    return (
      <AdminLayout>
        <div className="flex h-80 flex-col items-center justify-center rounded-[2rem] border border-slate-200 bg-white px-5 text-center shadow-sm">
          <p className="text-lg font-black text-slate-950">Remote Host not found</p>
          {actionError ? (
  ...
  const authLabel = host.sshAuthMode === "password" ? "SSH password" : "SSH private key";
  const gatewayLabel = platformHost
    ? host.gatewayHost || host.sshHost || "Not configured"
    : "Masked operator credential";
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 max-w-2xl text-sm font-medium leading-relaxed text-slate-500">
              This is a personal / operator-managed host. Admin oversight is read-only, endpoint
              values stay masked, and credential or access mutations are unavailable.
            </p>
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <InfoRow label="Host id" value={host.id} />
              <InfoRow label="Management" value="Personal / operator managed" />
              <InfoRow
                label="Owner"
                value={host.ownerName || host.ownerEmail || host.ownerUserId || "Operator account"}
              />
              <InfoRow label="SSH target" value="Masked operator credential" />
              <InfoRow label="Gateway" value="Masked operator credential" />
              <InfoRow
                label="Created"
                value={host.createdAt ? formatDateTime(host.createdAt) : "Unknown"}
              />
            </div>
          </section>
        </div>
  ...
  return (
    <AdminLayout>
      <div className="space-y-7">
        ...
        <header className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {host.enabled === false ? "Enable host" : "Disable host"}
            </button>
            <button
              type="button"
        ...
        {confirmAction === "reset" ? (
          <DangerConfirmPanel
            title="Reset the pinned SSH host key?"
            description="Nora will refuse future Remote Docker operations until Test connection succeeds and pins the replacement key. Verify the host change out of band first."
        ...
        {confirmAction === "delete" ? (
          <DangerConfirmPanel
            title="Delete this platform host?"
            description="The registry row and its access grants will be removed. Active agents can block deletion; Nora will surface that conflict instead of silently orphaning them."
            expectedValues={[host.label, host.id]}
            value={confirmation}
            onChange={setConfirmation}
            onCancel={() => setConfirmAction(null)}
        ...
        <div className="flex flex-wrap gap-3" role="tablist" aria-label="Platform host detail">
          <TabButton
        ...
        {activeTab === "overview" ? (
          <div className="grid gap-6 xl:grid-cols-[1.1fr,0.9fr]">
            <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              <h2 className="text-lg font-black tracking-tight text-slate-950">Host overview</h2>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                />
                <InfoRow label="SSH target" value={remoteHostSshTarget(host)} />
                <InfoRow label="Gateway" value={gatewayLabel} />
                <InfoRow label="Authentication" value={authLabel} />
                <InfoRow
                  label="SSH host key"
                  value={host.sshHostKey ? "Pinned" : "Not pinned — run Test connection"}
                />
                <InfoRow
                  label="Last tested"
                  value={host.lastTestedAt ? formatDateTime(host.lastTestedAt) : "Never"}
                />
                <InfoRow
                  label="Updated"
                  value={host.updatedAt ? formatDateTime(host.updatedAt) : "Unknown"}
                />
              </div>
              {host.issue ? (
            ...
            <section className="rounded-[2rem] border border-brand-cyan/15 bg-brand-ink p-5 text-brand-foreground shadow-sm sm:p-6">
              <h2 className="text-lg font-black">Platform ownership</h2>
              ...
              <dl className="mt-6 space-y-4">
                ...
                <div className="rounded-2xl bg-brand-cyan/10 px-4 py-4">
                  ...
                  <dd className="mt-2 text-sm font-bold">{accessAvailabilityLabel(host, true)}</dd>
                </div>
                <div className="rounded-2xl bg-brand-cyan/10 px-4 py-4">
                  <dt className="text-[10px] font-black uppercase tracking-[0.16em] text-brand-cyan/70">
                    Registered
                  ...
                  <dd className="mt-2 text-sm font-bold">
                    {host.createdAt ? formatDateTime(host.createdAt) : "Unknown"}
                  </dd>
                </div>
              </dl>
            </section>
