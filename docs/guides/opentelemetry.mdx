...
<Note>
  This guide covers Nora's own **backend-level** telemetry — the exchange-granularity spans and
  metrics above, exported *from* the control plane to *your* observability stack. Per-LLM-call and
  per-tool-call sub-spans are a separate, OpenClaw-specific capability: agents push real
  OpenTelemetry traces *into* Nora's own native OTLP receiver, where they're stored, searched, and
  shown in the Traces lens on `/app/logs` — see the [logging control plane
  guide](/guides/logging#otlp-trace-ingest-and-the-traces-lens) for enabling it per workspace. It's
  OpenClaw-only (Hermes has no equivalent OTel export) and doesn't currently forward into the
  external stack this guide configures — the two systems are independent today. A2A Agent Cards
