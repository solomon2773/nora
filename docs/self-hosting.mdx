...
<Steps>
  ...
  <Step title="Follow the interactive prompts">
    ...
    6. Choose which runtime families to enable: **OpenClaw** (on by default) and **Hermes** (opt-in). Either can be declined, and only the enabled ones have their agent images built — declining OpenClaw skips building `nora-openclaw-agent:local` entirely. **NemoClaw** is a sandbox profile for OpenClaw agents, so it is only offered when OpenClaw is enabled. Declining every runtime family falls back to OpenClaw, since an install with none cannot deploy an agent.
    7. Choose access mode: **local only** (`http://localhost:8080`), **public domain behind a proxy**, or **public domain with TLS at Nginx**
    8. Optionally create a bootstrap admin account
    9. Start the stack; after login, add your LLM provider key in **Settings**
